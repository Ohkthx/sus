...
namespace SUS
{
    public static class CombatStage
    {
        ...
        private static void CombatTurn(Mobile initiator, ref List<string> log, Mobile attacker, Mobile target, bool extra = false)
        {
            ...
            #region Check Ranges
            int distance = Point2D.Distance(attacker, target);
            ...
            if (attacker.Weapon.Range < distance)
            {
                ...
                string text = $"{attacker.Name} moves towards {target.Name}";
                if (distance >= 1)
                {
            ...
            {   // Hit the target, need to calculate additional damage (if it was a critical)
                ...
                var damageDealt = target.Damage(atkDamage, attacker, isMagical: attacker.Weapon.IsMagical);
